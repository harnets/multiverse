...
#if PRINT_SYS_LOG
        if (_net_npu_id.net_npu_id == 0 || _net_npu_id.net_npu_id == 1)
...
// }
        ...
        while (_sim_time.sim_time < end_time)
        {
            ...
            while (fetch_elem(_ack_buf, 0, ack_p))
            { // if ack pkt is available
...
// if (ack_p.dst == _src.src) {// reaction for ACK/NACK packets
                ...
                else if (ack_p.pkt_type == PktType::NACK)
                { // NACK
                ...
                else if (ack_p.pkt_type == PktType::CNP)
                { // CNP
            ...
            if (ack_pkt_cnt == 0 && _snd_nxt.snd_nxt == _flow_size.flow_size)
            {
                break;
            }
            ...
            if (_flow_state == FlowState::UNCOMPLETE && _snd_nxt.snd_nxt < _flow_size.flow_size && _sim_time.sim_time >= _start_time.start_time && _pfc_state == PFCState::RESUME)
                             ...
                             NextHop &_next_hop, Seed &_seed)
    {
        ...
        while (_sim_time.sim_time < end_time)
        {
            if (!fetch_elem(_bid_pkt_buf.snd_buf, 0, pkt))
                break;
...
#endif
        // reset
        for (uint32_t i = 0; i < INPORT_NUM; i++)
        {
            _forward_plan.forward_plan[i] = 0;
        }
    ...
    //         _switch_id.switch_id, _local_port_id.local_port_id, _global_port_id.global_port_id);
        ...
        { // at most processing q_len packets
            ...
            {
                if (pkt.dst == 0)
                { // the pfc frame is received from the downstream egress port
                         ...
                         SimTimePerUpdate &_sim_time_per_update)
    {
        ...
        for (CountT i = _switch_id.switch_id * K_ARY; i < (_switch_id.switch_id + 1) * K_ARY; i++)
        {
            ...
            while (count_bit--)
            {
                if ((fwd_bitmap & (1 << num)) != 0)
                { // the packet should be forward to the e_port
                         ...
                         SimTime &_sim_time, SimTimePerUpdate &_sim_time_per_update, Seed &_seed)
    {
        ...
        {
            if (get_queue_len(_pkt_queue.pkt_buf[i]) <= 0)
                continue;
            ...
            while (_sim_time.sim_time < end_time)
            {
                ...
                if (!fetch_elem(_pkt_queue.pkt_buf[i], 0, pkt))
                    break;
                if (pkt.enqueue_time >= end_time)
                    break;
                            ...
                            SimTime &_sim_time, SimTimePerUpdate &_sim_time_per_update)
    {
        ...
        while (_sim_time.sim_time < end_time)
        {
            if (!fetch_elem(_bid_pkt_buf.recv_buf, 0, pkt))
                break;
            if (pkt.enqueue_time >= end_time)
                break;
...
// if (_flow_id.flow_id == 0) {
        ...
        while (_sim_time.sim_time < end_time)
        {
            ...
            if (!fetch_elem(_recv_queue, 0, pkt))
                break;
            ...
            if (pkt.pkt_type != PktType::DATA)
            { // ACK/NACK is received by sender's receiver
                Entity snd_flow_entt = ctx.data().snd_flows[pkt.dst][pkt.flow_id];
                if (snd_flow_entt == Entity::none())
                {
                    return;
                }
...
// printf("flow_id: %d\n", _flow_id.flow_id);
            ...
            { // DATA pkt is received by receiver
                uint16_t pkt_size = pkt.header_len + pkt.payload_len;
...
// printf("src: %d recv: %d, expected_recv_bytes: %ld\n", recv_flow_queue.recv_flow[0].src, recv_flow_queue.recv_flow[0].dst, _recv_bytes.recv_bytes);
                ...
                int64_t sq_num;
                Pkt ack_pkt;
                if (pkt.sq_num == _recv_bytes.recv_bytes)
                { // packet order is right: generate ACK
                ...
                if (recv_snd_flow_entt == Entity::none())
                {
                    return;
                }
